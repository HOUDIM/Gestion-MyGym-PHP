...
<body bgcolor="#999999">
	<form method="post" action="" enctype="multipart/form-data">
		<table width="794px" align="center" border="1" bgcolor="#f1533e">
			<tr>
				<td colspan="2" align="center"><h1>Modifier ou mettre à jour des exercices</h1></td>
			</tr>
			<tr>
				<td align="right"><b>Nom d'utilisateur</b></td>
				<td>
					<select name="user">
			...
			<tr>
				<td align="right"><b>Nombre des Series</b></td>
			...
			<tr>
				<td colspan="2" align="center"><input type="submit" name="update_workout" value="Mettre Ajour L'exercice"></td>
			</tr>
		</table>
	</form>
</body>
...
<?php 
	...
	if (isset($_POST['update_workout']))
	{
		...
		//Validations
		if($user_name==''){
			echo "<script>alert('Remplir Tous Les Champ Svp!')</script>";
			exit();
		}
		elseif ($exer_name=='') {
			echo "<script>alert('Remplir Tous Les Champ Svp!')</script>";
		...
		elseif ($exercise_img=='') {
			...
			if ($run_update) {
				echo "<script>alert('Exercise a été mis A jour')</script>";
				echo "<script>window.open('index.php?view_exercises','_self')</script>";
			}
			else{
				echo "<script>alert('erreu veuillez ressayer')</script>";
			}
